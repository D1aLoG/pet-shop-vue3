<template>
  <div class="mx-auto w-full max-w-4xl">
    ...
    <div v-else-if="products.length === 0" class="py-16 text-center text-slate-500">
      ...
      <router-link to="/" class="mt-4 inline-block text-purple-600 hover:underline">
        Перейти в каталог
      </router-link>
    </div>

    <div v-else class="flex flex-col gap-6">
      <CartProduct v-for="(product, index) in products" :key="product.id" :item="product"
                   :index="index" />

      <div
        class="mt-4 flex flex-col items-end gap-4 self-end rounded-lg border bg-white p-6 shadow-sm">
