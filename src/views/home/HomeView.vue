<template>
  <div class="flex w-full flex-col gap-8">
    <div class="flex flex-col items-center justify-between gap-4 md:flex-row">
      ...
      <div class="flex flex-col items-stretch gap-2 sm:flex-row sm:items-center">
        <select v-model="selectedAnimal" class="rounded-lg border bg-white p-2 text-slate-700">
          ...
          <option v-for="animal in uniqueAnimals.filter(a => a !== 'all')" :key="animal"
                  :value="animal">
            {{ animal }}
          </option>
        </select>
        ...
        <input
          ...
          class="rounded-lg border bg-white py-2 pl-4 pr-4 text-slate-700 outline-none focus:ring-2 focus:ring-purple-500"
    ...
    <div v-else
         class="grid grid-cols-2 gap-4 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5">
      <Product v-for="product in displayProducts" :key="product.id" :item="product" />

      <div v-if="!pending && displayProducts.length === 0"
